...
public class CatalogueIdentifierHandler implements Handler<RoutingContext> {

    private final DatasetManager datasetManager;

    public CatalogueIdentifierHandler(DatasetManager datasetManager) {
    ...
    public void handle(RoutingContext routingContext) {
        ...
        if (parameters.queryParameter("catalogue") == null) {
            datasetManager.catalogue(uriRef.getUriRef())
                    .onSuccess(resource -> {
                        routingContext.put("catalogueId", DCATAPUriSchema.parseUriRef(resource.getURI()).getId());
                        routingContext.next();
                    })
                    .onFailure(cause -> {
                        if (cause instanceof TripleStoreException tse && tse.getCode() == 404) {
                            HubRepo.failureResponse(routingContext, new ServiceException(404, "Dataset not found"));
                        } else {
