...
export default {
  ...
  data () {
	...
	legend: false,
        legendCallback: (chart) => {
            var text = [];
                  text.push('<ul class="ul-line-chart-category">');
                  for (let i=0; i<chart.data.datasets.length; i++) {
                      let classname = "line-chart-category-clickable-legend-text-"+i
                      text.push('<li class="line-chart-category-clickable-legend">');
  ...
  mounted () {
    ...
    this.renderChart({
      ...
      document.getElementById('line-chart-category-js-legend').innerHTML = this.generateLegend();
      let chart = this.$data._chart
          ...
          if (alreadyHidden) {
              ci.getDatasetMeta(index).hidden = null;
              document.getElementById("line-chart-category-clickable-legend-text-"+index).classList.remove("hidden");
          } else {
              // otherwise, lets figure out how to toggle visibility based upon the current state
              ci.data.datasets.forEach(function (e, i) {
                  var meta = ci.getDatasetMeta(i);
                  document.getElementById("line-chart-category-clickable-legend-text-"+i).classList.add("hidden");
                  if (i !== index) {
                      // handles logic when we click on visible hidden label and there is currently at least
                      // one other label that is visible and at least one other label already hidden
                      // (we want to keep those already hidden still hidden)
                      if (anyOthersAlreadyHidden && !allOthersHidden) {
                          ...
                          meta.hidden = meta.hidden === null ? !meta.hidden : null;
                          if (meta.hidden == null){
                              document.getElementById("line-chart-category-clickable-legend-text-"+i).classList.remove("hidden");
                          }
                      }
                  } else {
                      meta.hidden = null;
      ...
      const legendItems = [
          ...document.querySelectorAll(".line-chart-category-clickable-legend")
      ];
