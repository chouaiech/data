<template>
  <div class="container">
    <div class="line-chart d-flex">
          <div id="line-chart-category-js-legend" class="col-4"></div>
     <line-chart
       class="col-8"     
      v-if="loaded"
...
import LineChart from './LineChartCategories.vue'


Vue.use(Trend)

export default {
  name: 'Evolution-Category',
  ...
  methods: {
    ...
    addLabels () {
      for (var i = 0; i < this.Catalogues.length; i++){
        for (var l = 0; l < this.Catalogues[i].stats.length; l++) {
            this.count_dummy.push(+this.Catalogues[i].stats[l].count)   //+ turns string into number
        }
      }
    ...
    round (num, nStelle) {
      num = (Math.round(num * nStelle) / nStelle)
      return num
    },
    requestData () {
      axios.get(this.$env.ROOT_API + '/data/ds-per-category?list=true', { // http://odp-lic.ppe-aws.europeandataportal.eu:9090/data/ds-per-catalogue'
      })
        .then(response => {
