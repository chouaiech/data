...
export default {
    ...
    return {
      options: {
        ...
        legendCallback: (chart) => {
          var text = [];
          text.push('<ul class="ul-line-chart-catalogue">');
          for (let i=0; i<chart.data.datasets.length; i++) {
              let classname = "line-chart-catalogue-clickable-legend-text-"+i
              text.push('<li title="' + chart.data.datasets[i].label + '" class="line-chart-catalogue-clickable-legend">');
  ...
  mounted () {
    ...
    this.chartlabels.forEach((cl, index) => {
      datasets.push({
          ...
          borderColor: this.chartLabelColors[index] || this.selectColor(index),
          backgroundColor: this.chartLabelColors[index] || this.selectColor(index),
          fill: false
        });
    });
    ...
    document.getElementById('line-chart-catalogue-js-legend').innerHTML = this.generateLegend();
    let chart = this.$data._chart;

    // make only first nine catalogues visible in chart (hide the rest)
    chart.data.datasets.forEach((e, i) => {
      if (i >= 9) {
        chart.getDatasetMeta(i).hidden = true;
        document.getElementById("line-chart-catalogue-clickable-legend-text-"+i).classList.add("hidden");
      }
    })
    chart.update();
    ...
    var onClick = function (e, index) {
        ...
        // remove this if you want the old behaviour
        var meta = ci.getDatasetMeta(index);
        if (meta.hidden) {
          meta.hidden = null;
          document.getElementById("line-chart-catalogue-clickable-legend-text-"+index).classList.remove("hidden");
        } else {
          meta.hidden = true;
          document.getElementById("line-chart-catalogue-clickable-legend-text-"+index).classList.add("hidden");
        }

        /* uncomment if you want the old behaviour and remove above part
        var alreadyHidden = (ci.getDatasetMeta(index).hidden === null) ? false : ci.getDatasetMeta(index).hidden;
        var anyOthersAlreadyHidden = false;
        var allOthersHidden = true;
            ...
            ci.data.datasets.forEach(function (e, i) {
                ...
                if (i !== index) {
                    ...
                    if (anyOthersAlreadyHidden && !allOthersHidden) {
                        ...
                        meta.hidden = meta.hidden === null ? !meta.hidden : null;
                        if (meta.hidden == null){
                            document.getElementById("line-chart-catalogue-clickable-legend-text-"+i).classList.remove("hidden");
                        }
                    }
                } else {
                    meta.hidden = null;
    ...
    const legendItems = [
        ...document.querySelectorAll(".line-chart-catalogue-clickable-legend")
    ];
  ...
  methods: {
    selectColor(number) {
      const hue = (number/3) * 137.508;
      const saturation = (number%65)+25;
      const lightness = (number%65)+20;
